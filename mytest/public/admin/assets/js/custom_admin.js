...
(function($)
{
	...
	// 	$.ajax({
	// 		type: "POST",
	// 		url: BASE_URL + 'admin/transaction/add/',
	...
	// });
		...
		$(document).ready(function () {
			$(".province_ajax").change(function () {
				var id = $(this).val();
				$.ajax({
					type: "POST",
					url: BASE_URL + 'order/district/' + id,
					data: "ajax",
					async: true,
		...
		$(document).ready(function () {
			$(".typecata").change(function () {
				var id = $(this).val();
				$.ajax({
					type: "POST",
					url: BASE_URL + 'admin/catalog/type/' + id,
					data: "ajax",
					async: true,
					success: function (data) {
						$(".parentcata").html(data);
					}
				})
			})
		});
		...
		$('a.verify_action1').click(function(){
		...
		$('a.verify_action2').click(function(){
		...
		$list_action.find('#submit').click(function(){ //tim toi the co id = submit,su kien click
			...
			$.ajax({
				...
				{
					var obj_deleted = msg.deleted; 
					var obj_error = msg.error;
					
					if(obj_deleted.length){
						$.each(obj_deleted, function(key, val)
						{
					...
					if(obj_error.length){
						alert('Có ' + obj_error.length + ' không xóa được!');
					}
				}
